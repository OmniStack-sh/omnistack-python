...
The REST API documentation can be found on [docs.omnistack.com](https://docs.omnistack.com). The full API of this library can be found in [api.md](api.md).
...
While you can provide a `jwt_bearer_token` keyword argument,
we recommend using [python-dotenv](https://pypi.org/project/python-dotenv/)
to add `JWT_BEARER_TOKEN="My Jwt Bearer Token"` to your `.env` file
so that your Jwt Bearer Token is not stored in source control.
...
async def main() -> None:
    completion = await client.openai.completions.create(
        model="string",
        prompt="string",
    )
...
client.with_options(max_retries=5).openai.completions.create(
    model="string",
    prompt="string",
)
```
...
client.with_options(timeout=5.0).openai.completions.create(
    model="string",
    prompt="string",
)
```
...
completion = response.parse()  # get the object that `openai.completions.create()` would have returned
print(completion)
```
